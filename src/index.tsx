...
function loadProjectContext(): void {
  ...
  if (fs.existsSync(nexusFile)) {
    ...
    setConfig(
      'systemPrompt',
      `You are nexus, an expert AI coding assistant. The user has provided this project context:\n\n${content}\n\nUse this context to give better, more relevant answers.`,
    );
  }
}
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if ((arg === '--provider' || arg === '-p') && args[i + 1]) {
      setConfig('provider', args[i + 1] as 'openai' | 'anthropic' | 'openrouter' | 'custom');
      i++;
